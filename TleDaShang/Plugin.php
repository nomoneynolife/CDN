...
class TleDaShang_Plugin implements Typecho_Plugin_Interface{
    ...
    public static function config(Typecho_Widget_Helper_Form $form){
		...
		//QQ、微信、支付宝链接设置
		$qqUrl = new Typecho_Widget_Helper_Form_Element_Text('qqUrl', array('value'), 'https://i.qianbao.qq.com/wallet/sqrcode.htm?m=tenpay&f=wallet&u=584764651&a=1&n=_&ac=CAEQ65nrlgIY-8O9-gU%3D_xxx_sign', _t('QQ支付二维码url'), _t('可使用<a href="https://cli.im/deqr/" target="_blank">草料二维码</a>将二维码图片转成url地址填入其中'));
        $form->addInput($qqUrl);
		$wechatUrl = new Typecho_Widget_Helper_Form_Element_Text('wechatUrl', array('value'), 'wxp://f2f0ggypEuYzt_WQgBxnJov3AP5_moZXDBlD', _t('微信支付二维码url'), _t('可使用<a href="https://cli.im/deqr/" target="_blank">草料二维码</a>将二维码图片转成url地址填入其中'));
        $form->addInput($wechatUrl);
		$aliUrl = new Typecho_Widget_Helper_Form_Element_Text('aliUrl', array('value'), 'https://qr.alipay.com/fkx16932xf0hsy6nvzhc0c5', _t('支付宝支付二维码url'), _t('可使用<a href="https://cli.im/deqr/" target="_blank">草料二维码</a>将二维码图片转成url地址填入其中'));
		...
		//独立页面打赏设置
		$tledashangqq = new Typecho_Widget_Helper_Form_Element_Text('tledashangqq', array("value"), '10000', _t('QQ号'), _t('通过QQ号自动获取头像地址和联系QQ链接'));
		...
		$tledashangaudiourl = new Typecho_Widget_Helper_Form_Element_Text('tledashangaudiourl', array("value"), 'https://m3.8js.net:99//1858/051204580539587.mp3', _t('乞讨歌Url'), _t('输入乞讨歌的Url地址'));
